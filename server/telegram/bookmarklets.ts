...
function buildSendFunction(apiVar: string): string {
  return `function sendToApi(payload,onSuccess,onFail){` +
    `var done=false;` +
    `try{` +
      `var x=new XMLHttpRequest();` +
      `x.open('POST',${apiVar},true);` +
      `x.setRequestHeader('Content-Type','application/json');` +
      `x.onload=function(){if(done)return;done=true;try{var d=JSON.parse(x.responseText);onSuccess(d)}catch(e){onFail('parse')}};` +
      `x.onerror=function(){if(done)return;done=true;` +
        `fetch(${apiVar},{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),mode:'cors'})` +
        `.then(function(r){return r.json()})` +
        `.then(function(d){onSuccess(d)})` +
        `.catch(function(){` +
          `var img=new Image();img.src=${apiVar}+'?beacon=1&d='+encodeURIComponent(JSON.stringify(payload));` +
          `onFail('blocked')` +
        `})` +
      `};` +
      `x.timeout=15000;` +
      `x.ontimeout=function(){if(done)return;done=true;onFail('timeout')};` +
      `x.send(JSON.stringify(payload))` +
    `}catch(e){if(!done){done=true;onFail('error')}}}`;
}
...
export function generateLinkedInBookmarkletCode(baseUrl: string, chatId: string, businessId: number, token: string): string {
  ...
  const sendFn = buildSendFunction('API');
  const code = `javascript:void((function(){` +
    ...
    `var CID='${chatId}',BID=${businessId},TOK='${token}',API='${apiUrl}';` +
    ...
    `closeBtn.onclick=function(){banner.remove();window.__geminEyeLiActive=false;autoScrolling=false;clearInterval(si);clearInterval(scrollInterval)};` +
    ...
    `sendToApi(payload,function(d){pendingCount--;if(d.matched){sentCount++;el.style.outline='3px solid #0077B5';el.style.outlineOffset='4px';el.style.borderRadius='4px'}updateCounter()},function(reason){pendingCount--;failCount++;updateCounter();if(failCount===1)console.error('Gemin-Eye: requests blocked by page CSP. Reason:',reason)})}` +
...
export function generateBookmarkletCode(baseUrl: string, chatId: string, businessId: number, token: string): string {
  ...
  const code = `javascript:void((function(){` +
    ...
    `var closeBtn=document.createElement('span');closeBtn.textContent='[X] Close';` +
    `closeBtn.style.cssText='cursor:pointer;background:rgba(255,0,0,0.35);padding:4px 12px;border-radius:4px;font-size:12px;font-weight:700;margin-left:4px;';` +
    `closeBtn.onclick=function(){banner.remove();window.__geminEyeActive=false;autoScrolling=false;clearInterval(si);clearInterval(scrollInterval)};` +
    ...
    `sendToApi(payload,function(d){pendingCount--;if(d.matched){sentCount++;el.style.outline='3px solid #6d28d9';el.style.outlineOffset='4px';el.style.borderRadius='4px'}updateCounter()},function(reason){pendingCount--;failCount++;updateCounter();if(failCount===1)console.error('Gemin-Eye: requests blocked by page CSP. Reason:',reason)})}` +
    `function scan(){var posts=extractPosts();if(posts.length===0)noNewCount++;else noNewCount=0;posts.forEach(function(p){if(scannedCount>=500)return;sendPost(p.text,p.element)});if(scannedCount>=500||noNewCount>=10){autoScrolling=false;clearInterval(scrollInterval);updateCounter()}}` +
    `scan();var si=setInterval(scan,2000);` +
    `var scrollInterval=setInterval(function(){if(!autoScrolling)return;scrollsDone++;if(scrollsDone>=maxScrolls){autoScrolling=false;clearInterval(scrollInterval);updateCounter();return}window.scrollBy({top:600,behavior:'smooth'})},1500)` +
    `})())`;
