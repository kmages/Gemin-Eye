...
- **Dynamic App Base URL**: All Telegram bot links (dashboard, onboarding, daily reminder buttons) now flow through `getAppBaseUrl()` in `server/telegram/bookmarklets.ts`. Resolution order: `APP_BASE_URL` env var → `https://gemin-eye.com` if `NODE_ENV=production` or `REPLIT_DEPLOYMENT=1` → first entry of `REPLIT_DOMAINS` for dev → `https://gemin-eye.com` fallback. This means dev Telegram messages link to the dev preview, not production.
